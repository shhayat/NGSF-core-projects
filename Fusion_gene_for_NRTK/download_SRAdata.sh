...
do
  ...
  ${sratoolkit}/fastq-dump --outdir $OUTPUT/fastq --gzip --skip-technical --readids --read-filter pass --split-3 --clip ${OUTPUT}/${i}/${i}.sra;
...
do
  ...
  echo "Generating fastq for: ${i}";
  ${sratoolkit}/fastq-dump --outdir $OUTPUT/fastq --gzip --clip ${OUTPUT}/${i}/${i}.sra;
done
