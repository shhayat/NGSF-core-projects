...
df.wide <- cbind(df.wide.response,df.wide.scores[2:length(df.wide.scores)])
df.wide <- df.wide[order(df.wide$conc2),]
df.wide <- df.wide[c(1:4,12,5,13,6,14,7,15,11,8,16,9,17,10,18,19:21,22:24,32,33,25,26,34,27,35,31,28,36,29,37,30,38)]
...
#For MDA_MB_157                
dir.create("MDA_MB_157", recursive=TRUE, showWarnings = FALSE) 
df <- read.csv("Paclitaxel_Homoharringtonine_MDA_MB_157.csv", head=TRUE)[1:8]
#use first row as column name
names(df) <- as.matrix(df[1, ])
#remove first row from df and select rest of the rows
df1 <- df[-1, ][1:35,]
...
write.csv(synergy.score, "MDA_MB_157/MDA_MB_157_synergy_scores.csv")
save(synergy.score, file = 'MDA_MB_157_synergy.score.RData', compress = 'xz')                

 
#2D contour plot

jpeg("MDA_MB_157/MDA_MB_157_2D_contour_plot_paclitaxel_homoharringtonine.jpeg", width=700, height=600)
Plot2DrugContour(
  data = res,
  plot_block = 1,
  drugs = c(1, 2),
...
#Plot synergy scores
jpeg("MDA_MB_157/MDA_MB_157_plot_synergyscores.jpeg",width=950, height=600)
...
#round dose response values to whole number
res$response$response <- round(res$response$response)
save(res, file = 'MDA_MB_157_res.RData', compress = 'xz')
